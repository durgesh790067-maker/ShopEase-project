...
if ! "$MYSQL_BIN/mysqladmin" --socket="$MYSQL_SOCKET" ping --silent 2>/dev/null; then
    echo "Starting MySQL..."
    "$MYSQL_BIN/mysqld" --datadir="$MYSQL_DATADIR" \
           --socket="$MYSQL_SOCKET" \
           --port="$MYSQL_PORT" \
...
"$MYSQL_BIN/mysql" --socket="$MYSQL_SOCKET" -u root --connect-expired-password 2>/dev/null <<ENDSQL
...
DB_EXISTS=$("$MYSQL_BIN/mysql" --socket="$MYSQL_SOCKET" -u root -p"${MYSQL_ROOT_PASS}" -e "SELECT SCHEMA_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME='quickcart';" 2>/dev/null | grep -c quickcart)
if [ "$DB_EXISTS" -eq "0" ]; then
    echo "Importing database schema..."
    "$MYSQL_BIN/mysql" --socket="$MYSQL_SOCKET" -u root -p"${MYSQL_ROOT_PASS}" < /home/runner/workspace/QuickCart/quickcart.sql 2>/dev/null
    echo "Database imported successfully."

    echo "Applying customizations..."
    "$MYSQL_BIN/mysql" --socket="$MYSQL_SOCKET" -u root -p"${MYSQL_ROOT_PASS}" QuickCart 2>/dev/null <<CUSTOMSQL
...
echo "Starting PHP server on port ${PORT:-5000}..."
exec php -S 0.0.0.0:${PORT:-5000} -t /home/runner/workspace/QuickCart/
